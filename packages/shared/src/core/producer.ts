import type { Line, ProducerItem } from '@root/types'
import type { ContextCommon } from '@root/types'

import { DEFAULT_PRODUCER_RULES, DEFAULT_PRODUCER_RULES_QUICK_KEYWORDS, PRODUCER_MATCH_MODE } from '@root/constants'
...
export const processProducer = (context: ContextCommon, infos: Line.Info[]): [Line.Info[], ProducerItem[]] => {
  const options = context.options.common.getByKey('meta.producer')
  if (!options.enable) {
    return [infos, []]
  }
